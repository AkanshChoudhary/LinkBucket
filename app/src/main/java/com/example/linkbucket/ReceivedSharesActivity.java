...
public class ReceivedSharesActivity extends AppCompatActivity {
    ...
    List<HistoryItem> receivedList = new ArrayList<>();
    ArrayList<ArrayList<String>> listOfAllLists = new ArrayList<>();
    ...
    public void addCardsToAccount(int position,String collName){
        ...
        FirebaseFirestore.getInstance().collection("user+" + senderUserIds.get(position)).document("allSavedThings").collection("cards")
                ...
                .addOnCompleteListener(task -> {
                    ...
                    int j = MainUserDashboard.finalCardNumber + 1;
                    for (int i = 0; i < myList.size(); i++) {
                        ...
                        addMap.put("cardNumber", j);
                        addMap.put("upload_time", timestamp.getTime());
                        DocumentReference dc = FirebaseFirestore.getInstance().collection("user+" + FirebaseAuth.getInstance().getCurrentUser().getUid()).document("allSavedThings").collection("cards").document("card " + j);
                        batch.set(dc, addMap);
                        cardNumbers.add("card "+j);
                        j = j + 1;
                    }
