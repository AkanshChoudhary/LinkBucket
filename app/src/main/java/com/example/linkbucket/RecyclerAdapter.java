...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapter.ViewHolder> {
    ...
    public interface OnCardSelectedListener {
        void onCardSelected(long cardNumber);
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (recyclerItem.getFlag() == 1) {
            holder.v.setAlpha((float) 0.3);
        } else if (recyclerItem.getFlag() == -1) {
            holder.v.setAlpha(1);
        }
    }
        ...
        public ViewHolder(@NonNull View itemView, OnCardSelectedListener listener, OnCardClickedListener listener2,OnMoreClickedListener listener3) {
            ...
            itemView.setOnLongClickListener(v -> {
                if (itemView.getAlpha() == (float) 0.3) {
                    itemView.setAlpha(1);
                    recyclerItemList.get(getAdapterPosition()).changeState(false);
                    if (flag == 0) {
                    ...
                    recyclerItemList.get(getAdapterPosition()).changeState(true);
                    if (flag == 0) {
